...
- **対象のアプリが最前面のときだけ送ります**（既定ON）。
  ARKを見ていないときに勝手にキーが飛ばないので、そのままにしておくのがおすすめです。
  対象は「いま最前面のを使う」ボタンで自動的に入ります
- 「▶ ためす」でその場に1回送って動作確認できます
- **起動時は必ず停止状態**から始まります

<img src="docs/afk.png" width="640">

> 自分で建てているサーバーなら、`GameUserSettings.ini` の放置キック設定を切るほうが確実です。
> キー送信はクライアント側の対処なので、ARKが後ろに回っている間は届きません。
